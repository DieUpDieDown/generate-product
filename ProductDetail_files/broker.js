...
var _reg = /(^http(s)?\:\/\/www\.microsoftstore\.com\/store\/msusa\/en_us\/(cat\/yournextpc|list\/categoryid))|(windows\/en-us\/honestly)|(en-us\/(windows-8\/(meet|compare)|windows\/pc-selector))/i.test(document.referrer.toString().toLowerCase());
var _cleRef = /^http(s)?\:\/\/(i3|js|www|i)\.microsoft\.com\/library\/svy\/msftstore\/int_cle\.htm/i.test(document.referrer);
var _svRef = /^http(s)?\:\/\/survey2\.surveysite\.com\/wix/i.test(document.referrer);
function getCookieVal(name){var ca=document.cookie.split(';');var nameEQ=name+"=";for(var i=0;i<ca.length;i++){var c=ca[i];while(c.charAt(0)==' ')c=c.substring(1,c.length);if(c.indexOf(nameEQ)==0)return c.substring(nameEQ.length,c.length)}return false;}

if(/^http(s)?\:\/\/(i3|js|www|i)\.microsoft\.com\/library\/svy\/msftstore\/int_cle\.htm/i.test(SR_url)){ 
...
var _rn = getRandom();
var checkCle = getCookieVal("cleflag");
function loadCle(st, freq,_s,_l) {
		_freq = freq;
		var cleForce=false;
		if(getCookieVal("tstCLE")==1){var c='tstCLE=;expires=Thu, 01 Jan 1970 00:00:01 GMT;';	document.cookie=c; cleForce=true; _freq=100;}
		if(!checkCle || cleForce) {
			if(!(/^http(s)?\:\/\/www\.microsoftstore\.com/i.test(document.referrer))){ 
				if(_rn <= _freq){					
					if(st==1) {
						window.location.href = 'http://i3.microsoft.com/library/svy/msftstore/int_cle.htm?location='+escape(SR_url)+'&referrer='+_refv+'&frequency='+_freq+'&weight=100&site='+_s+'&SURVEY_TYPE=1&l='+_l;
					}					
				}
		}
	}
	...
	if(!(/iphone|ipad|iphone|android|opera mini|blackberry|windows(phone|ce)|iemobile|htc|nokia/i.test(navigator.userAgent))){
		if(!getCookieVal("msresearch")){
 			if(/[\w\.]+\/store\/msusa\/en_US/i.test(SR_url)){
				var _f = 0.0102, _l="9";
				if(/list\/categoryID\.67297700#\/filter=Tablets/i.test(SR_url)) {
					loadCle(1, _f,"2370",_l);	
				}else if(/list\/categoryID\.67297700#\/filter=Laptops/i.test(SR_url)) {
					loadCle(1, _f,"2371",_l);	
				}else if(/list\/categoryID\.67297700#\/filter=2-in-1%2BPCs/i.test(SR_url)) {
					loadCle(1, _f,"2372",_l);	
				}else if(/list\/categoryID\.67297700#\/filter=All-in-one%2BPCs/i.test(SR_url)) {
					loadCle(1, _f,"2373",_l);	
				}else if(/cat\/YourNextPC\/categoryID\.67297800#Office-Computers/i.test(SR_url)) {
					loadCle(1, _f,"2374",_l);	
				}else if(/cat\/YourNextPC\/categoryID\.67297800/i.test(SR_url)) {
					loadCle(1, 0.5,"2368",_l);
				}else if(/list\/categoryID\.67297700/i.test(SR_url)) {
					loadCle(1, _f,"2369",_l);	
				}
 			}
 		}
	}
...
if(!checkCle){
	var c = 'cleflag=1; path=/; domain=.microsoftstore.com';
	document.cookie = c;	
}
if(_reg){
	var _v = getCookieVal('cleflag'); _v++;
	var c = 'cleflag='+_v+'; path=/; domain=.microsoftstore.com';
	document.cookie = c;	
}
		...
		return {
			...
			*/
			...
			start: function() {
				...
				if(/iphone|ipad|ipod|android|opera (mini|mobi)|blackberry|windows (phone|ce)|iemobile|htc|nokia|mobile/i.test(navigator.userAgent) || (_sr_config=="broker-config.js" && _reg)){
				//if(/iphone|ipad|ipod|android|opera (mini|mobi)|blackberry|windows (phone|ce)|iemobile|htc|nokia|mobile/i.test(navigator.userAgent) || _reg || _cleRef || getCookieVal('cleflag')>1 )
